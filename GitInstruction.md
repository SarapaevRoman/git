...
* Необходимо выполить привязку локального репозитория с удаленным, для этот нам необходимо команда `remote` с указанием пути. 
Наприменр:
```
 ...
 * Для отправки изминений в удаленный репозиторий используется команда  `push`
```
git push -u <имя для репозитория> main
```
 Изменение в репозитории можно в носить в GitHub, для фиксации после изменениия необходимо нажать на кнопку ` commit changes` и указать комментарий к коммиту.
 * Для получения изменений с удаленного репозитория используется команда `git pull`